...
describe('extractExtensionId', () => {
  it('should extract Chrome extension ID from new URL format', () => {
    ...
    expect(result).toEqual({ id: 'aapbdbdomjkkjkaonfhkkikfgjllcleb', store: 'chrome', isRawId: false });
  });
  ...
  it('should extract Edge extension ID', () => {
    ...
    const result = extractExtensionId(url);
    expect(result).toEqual({ id: 'odlbpnoocpeebfbbnocajebccdbogpbe', store: 'edge', isRawId: false });
  });
  ...
  it('should extract ID from raw string and default to Edge', () => {
    ...
    expect(result).toEqual({ id: 'pnmchffiealhkdloeffcdnbgdnedheme', store: 'edge', isRawId: true });
  });
});
