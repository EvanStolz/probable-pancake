...
import { Shield, AlertTriangle, Info, Skull, Star, Users, Calendar, User, Code, Plus, Minus } from 'lucide-react';

interface AnalysisResultsProps {
  result: AnalysisResult;
}

function ExpandableSection<T>({ items, limit = 5, children, emptyMessage }: { items: T[], limit?: number, children: (items: T[]) => React.ReactNode, emptyMessage?: React.ReactNode }) {
    ...
    <div className="w-full">
      ...
      {hasMore && (
        <button
          ...
          className="flex items-center gap-1 text-sm font-bold text-blue-500 hover:text-blue-600 transition-colors mt-2"
        >
          {isExpanded ? (
            <><Minus className="w-4 h-4" /> Show Less</>
          ) : (
            <><Plus className="w-4 h-4" /> Show {items.length - limit} More</>
          )}
        </button>
      )}
    </div>
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <ExpandableSection
          ...
          emptyMessage={<p className="text-zinc-500 italic p-4 bg-zinc-50 dark:bg-zinc-800 rounded-lg text-center">No special permissions requested</p>}
        >
          {(items) => (
      ...
      <section>
        ...
        <ExpandableSection
          ...
          emptyMessage={<p className="text-zinc-500 italic p-4 bg-zinc-50 dark:bg-zinc-800 rounded-lg text-center">No known vulnerabilities detected</p>}
        >
          {(items) => (
            <div className="grid gap-4">
              {items.map((v, idx) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <ExpandableSection
              ...
              emptyMessage={<p className="text-zinc-500 italic text-sm">No third-party libraries identified</p>}
            >
              {(items) => (
          ...
          <div className="space-y-4">
            ...
            <ExpandableSection
              items={result.apiCalls}
              emptyMessage={<p className="text-zinc-500 italic text-xs">No sensitive API calls detected</p>}
            >
              {(items) => (
              ...
              <div className="space-y-4">
                {result.obfuscationTriggers.map((trigger, idx) => (
                  <div key={idx} className="border-l-2 border-red-500 pl-4 py-1">
                    ...
                    <ExpandableSection items={trigger.files}>
                      {(files) => (
